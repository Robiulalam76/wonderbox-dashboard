        ...
        <div style={{ overflowY: "auto", maxHeight: "500px" }}>
          {allNotification.length > 0 &&
            allNotification?.map((notify) => (
              <div
                key={notify?._id}
                className="d-flex justify-content-center px-4 py-3 border-bottom mx-2"
              >
                ...
                <p className="text-gray-600 text-sm mx-2">
                  <span className="fw-bold">
                    {notify?.type === "new_user" && (
                      <p className="text-blue-600 fw-bold">Congratulations! </p>
                    )}
                    {notify?.title}{" "}
                    <span className="text-uppercase fw-bold text-blue-500">
